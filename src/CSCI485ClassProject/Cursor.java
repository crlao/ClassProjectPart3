...
public class Cursor {
  ...
  public Cursor(Mode mode, String tableName, TableMetadata tableMetadata, Transaction tx, String indexAttr) {
    this.mode = mode;
    this.tableName = tableName;
    this.tableMetadata = tableMetadata;
    this.tx = tx;
    this.indexAttr = indexAttr;
    this.indexSubspace = FDBHelper.openSubspace(tx, Indexes.getIndexPath(tx, tableName, indexAttr));
  }
  ...
  private Record moveToNextRecord(boolean isInitializing) {
    ...
    if (isInitializing) {
      ...
      fdbIterable = FDBHelper.getKVPairIterableOfDirectory(directorySubspace, tx, isInitializedToLast);
    ...
    if (directorySubspace == null || (indexAttr != null && indexSubspace == null) || !hasNext()) {
      return null;
    }
  ...
  public StatusCode updateCurrentRecord(String[] attrNames, Object[] attrValues) {
    ...
    for (FDBKVPair kv : kvPairsToUpdate) {
      ...
      if (indexAttr != null) {
        List<String> indexPath = Indexes.getIndexPath(tx, tableName, indexAttr);
        Tuple key = new Tuple();
        Tuple val = new Tuple();
        key.addObject(currentRecord.getValueForGivenAttrName(indexAttr, indexPath.get(3) == "hash"));
        for (String primaryKey : primaryKeys) {
          key.addObject(currentRecord.getValueForGivenAttrName(primaryKey));
        }
  ...
  public StatusCode deleteCurrentRecord() {
    ...
    if (indexAttr != null) {
      Tuple key = new Tuple();
      key.addObject(currentRecord.getValueForGivenAttrName(indexAttr, Indexes.getIndexPath(tx, tableName, indexAttr).get(3) == "hash"));
      for (String primaryKey : tableMetadata.getPrimaryKeys()) {
        key.addObject(currentRecord.getValueForGivenAttrName(primaryKey));
      }
      FDBHelper.removeKeyValuePair(indexSubspace, tx, key);
    }
