...
public class RecordsImpl implements Records{
  ...
  public List<String> getPrimaryKeys(Transaction tx, String tableName) {
    ...
    return tblMetadata.getPrimaryKeys();
  }
  ...
  public StatusCode insertRecord(String tableName, String[] primaryKeys, Object[] primaryKeysValues, String[] attrNames, Object[] attrValues) {
    ...
    for (String attrName: attrNames) {
      List<String> indexPath = Indexes.getIndexPath(tx, tableName, attrName);
      indexPath.add(attrName);
      if (!indexPath.isEmpty()) {
        DirectorySubspace indexSubspace = FDBHelper.createOrOpenSubspace(tx, indexPath);
        IndexType indexType = indexPath.get(indexPath.size()-1) == "hash" ? IndexType.NON_CLUSTERED_HASH_INDEX : IndexType.NON_CLUSTERED_B_PLUS_TREE_INDEX;

        Tuple key = new Tuple();
        Tuple val = new Tuple();

        key.addObject(record.getValueForGivenAttrName(attrName, indexType == IndexType.NON_CLUSTERED_HASH_INDEX));
        for (String primaryKey : schemaPks) {
          key.addObject(record.getValueForGivenAttrName(primaryKey));
        }
  ...
  public Cursor openCursor(String tableName, String attrName, Object attrValue, ComparisonOperator operator, Cursor.Mode mode, boolean isUsingIndex) {
    ...
    if (isUsingIndex && Indexes.getIndexPath(tx, tableName, attrName).isEmpty()) {
      FDBHelper.abortTransaction(tx);
      return null;
    }

    Cursor cursor = isUsingIndex ? new Cursor(mode, tableName, tblMetadata, tx, attrName) : new Cursor(mode, tableName, tblMetadata, tx);
